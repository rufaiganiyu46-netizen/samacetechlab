...
if (is_post()) {
    $user = attempt_login((string) ($_POST['email'] ?? ''), (string) ($_POST['password'] ?? ''), 'teacher', $error);

    if ($user) {
        set_flash('success', 'Welcome back, ' . (($user['first_name'] ?? '') ?: ($user['full_name'] ?? 'Teacher')) . '.');
        redirect('teacher-dashboard.php');
    }
}
?>
...
<head>
    ...
    <title>Teacher Login | SAMACE TECH LAB</title>
    ...
    <style>
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .orb-gold { width:500px; height:500px; background:radial-gradient(circle, rgba(245,166,35,0.12), transparent 70%); top:-100px; left:-100px; }
        .orb-teal { width:400px; height:400px; background:radial-gradient(circle, rgba(0,212,180,0.08), transparent 70%); bottom:0; right:0; animation-direction:alternate-reverse; animation-duration:25s; }
        .orb-blue { width:350px; height:350px; background:radial-gradient(circle, rgba(74,158,255,0.07), transparent 70%); top:50%; left:50%; animation-duration:30s; }
        ...
        .inline-status.success { background:rgba(0,212,180,0.12); color:#9ff4e7; }
        ...
        input:hover { background:rgba(255,255,255,0.07) !important; }
        ...
        .password-toggle:hover { color:var(--role-accent); background:var(--role-tint); }
        .form-meta { display:flex; justify-content:flex-end; margin-top:-4px; }
        .forgot-link {
            font-size:0.75rem;
            color:var(--role-accent);
            opacity:0.8;
        ...
        .submit-btn:hover { transform:translateY(-2px); box-shadow:0 0 30px var(--role-glow); }
        ...
        .card-links a { color:var(--text-muted); font-size:0.82rem; transition:var(--transition); }
        .card-links a:hover { color:var(--role-accent); }
...
<body class="auth-page" data-role="teacher" data-page="login">
    ...
    <main class="login-shell">
        <section class="login-card" id="loginCard">
            ...
            <p class="card-subtitle">Sign in to access teacher tools and features.</p>
            ...
            <div class="card-links">
                <a href="teacher-signup.php">Create Teacher Account</a>
