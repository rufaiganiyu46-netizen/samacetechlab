...
if (is_post()) {
    ...
    if ($password !== $confirmPassword) {
        ...
        $created = create_user([
            ...
            'password' => $password,
            'role' => 'teacher',
            'requested_teaching_class' => (string) ($_POST['class_name'] ?? ''),
        ], $error);

        if ($created) {
            set_flash('success', 'Teacher registration submitted. Wait for the principal to approve your account and assign your class before logging in.');
...
<head>
    ...
    <title>Teacher Sign Up | SAMACE TECH LAB</title>
    ...
    <style>
        :root {
            ...
            --text-muted: #5A6070;
            --role-accent: #4A9EFF;
            --role-glow: rgba(74,158,255,0.3);
            --role-tint: rgba(74,158,255,0.08);
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        body { background:#0A0A0F; min-height:100vh; font-family:var(--font-body); color:#fff; display:flex; align-items:center; justify-content:center; padding:40px 20px; overflow-x:hidden; position:relative; }
        ...
        .register-card { background:var(--bg-card); border:1px solid var(--border-glass); border-radius:24px; padding:52px 48px; backdrop-filter:blur(24px); -webkit-backdrop-filter:blur(24px); box-shadow:0 0 0 1px rgba(74,158,255,0.07), 0 32px 80px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.06); position:relative; overflow:hidden; transform-style:preserve-3d; animation:cardReveal .7s cubic-bezier(0.23,1,0.32,1) both; transition:transform .25s ease-out; }
        .register-card::before { content:''; position:absolute; top:0; left:0; right:0; height:3px; background:linear-gradient(90deg, transparent, rgba(74,158,255,0.9) 30%, rgba(0,212,180,0.45) 70%, transparent); border-radius:24px 24px 0 0; }
        .register-card::after { content:''; position:absolute; top:-60px; left:50%; transform:translateX(-50%); width:250px; height:130px; background:radial-gradient(circle, rgba(74,158,255,0.07), transparent 70%); pointer-events:none; }
        ...
        .corner-dot.tr { top:16px; right:16px; background:#00D4B4; box-shadow:0 0 10px rgba(0,212,180,0.8); animation-delay:.5s; }
        .corner-dot.bl { bottom:16px; left:16px; background:#4A9EFF; box-shadow:0 0 10px rgba(74,158,255,0.8); animation-delay:1s; }
        .corner-dot.br { bottom:16px; right:16px; background:#4A9EFF; box-shadow:0 0 10px rgba(74,158,255,0.8); animation-delay:1.5s; }
        .scan-line { position:absolute; left:0; right:0; height:1px; background:linear-gradient(90deg, transparent, rgba(74,158,255,0.35), transparent); animation:scanLine 4s ease-in-out infinite; pointer-events:none; z-index:1; }
        .school-brand { display:flex; align-items:center; gap:12px; margin-bottom:20px; position:relative; z-index:2; }
        .brand-icon { width:46px; height:46px; background:linear-gradient(135deg, rgba(74,158,255,0.2), rgba(74,158,255,0.05)); border:1px solid rgba(74,158,255,0.3); border-radius:12px; display:flex; align-items:center; justify-content:center; font-size:22px; box-shadow:0 0 20px rgba(74,158,255,0.15); }
        ...
        .portal-badge { display:inline-flex; align-items:center; gap:6px; background:rgba(74,158,255,0.1); border:1px solid rgba(74,158,255,0.25); color:var(--role-accent); font-size:11px; font-weight:600; letter-spacing:1.5px; text-transform:uppercase; padding:6px 14px; border-radius:50px; margin-bottom:20px; position:relative; z-index:2; }
        ...
        .form-input { width:100%; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); border-radius:14px; color:#fff; padding:14px 18px; font-family:var(--font-body); font-size:14px; transition:var(--transition); outline:none; min-height:52px; }
        .form-input::placeholder { color:#5A6070; }
        .form-input:focus { border-color:var(--role-accent); background:rgba(74,158,255,0.05); box-shadow:0 0 0 3px rgba(74,158,255,0.12), 0 8px 25px rgba(0,0,0,0.3); transform:translateY(-1px); }
        ...
        .btn-register { width:100%; margin-top:8px; background:linear-gradient(135deg, #4A9EFF, #2d7dd2); color:#0A0A0F; border:none; border-radius:14px; padding:16px; font-family:var(--font-display); font-size:20px; letter-spacing:2px; cursor:pointer; transition:var(--transition); position:relative; overflow:hidden; margin-bottom:24px; }
        ...
        .btn-register:hover { transform:translateY(-2px); box-shadow:0 0 40px rgba(74,158,255,0.4), 0 12px 30px rgba(0,0,0,0.4); }
        ...
        @keyframes floatDot { 0% { transform:translate(0,0) scale(1); opacity:.5; } 33% { transform:translate(12px,-18px) scale(1.3); opacity:1; } 66% { transform:translate(-8px,10px) scale(.7); opacity:.3; } 100% { transform:translate(18px,-25px) scale(1); opacity:.6; } }
        ...
        ::-webkit-scrollbar-thumb { background:rgba(74,158,255,0.4); border-radius:3px; }
...
<body class="auth-page" data-role="teacher" data-page="signup">
    ...
    <main class="register-wrapper">
        <section class="register-card">
            ...
            <p class="card-subtitle">Register as a teacher to access classroom and school management tools.</p>

            <?php if ($error): ?>
                <div class="inline-status error"><?php echo e($error); ?></div>
            <?php endif; ?>
            <?php if ($flash): ?>
                <div class="inline-status"><?php echo e((string) $flash['message']); ?></div>
            <?php endif; ?>
            <?php if ($db_error): ?>
            ...
            <form method="post" novalidate>
                <div class="form-grid">
                    ...
                    <div class="form-group full">
                        <label class="form-label" for="class_name">Preferred Class</label>
                        <input class="form-input" id="class_name" name="class_name" type="text" placeholder="e.g. Primary 3" value="<?php echo e((string) ($_POST['class_name'] ?? '')); ?>" required>
                    </div>
                    <div class="form-group full">
            ...
            <div class="card-footer-links">
                <a class="footer-link primary" href="teacher-login.php">Already have an account?</a>
